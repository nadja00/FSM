 ...
 *
 * Access Control FSM - Nested Switch (return) Pattern - ESP32
 * FSM: 4 states - IDLE, CHECKING, GRANTED, DENIED
 * Events: EV_VALID, EV_INVALID, EV_TIMEOUT
 *
 * Measurement: Xtensa CPU cycle counter via ESP.getCycleCount() (32-bit, runs at
 * full CPU clock, e.g. 240MHz -> much finer resolution than an AVR 16-bit timer).
 *
...
//inline se koristi kako se ne bi gubile performanse pri pozivu funkcije, da ne gubimo taktove, vec se ponasa kao da smo nalepili kod funkcije na mesto poziva
...
void setup() {
    ...
    delay(200);
    Serial.println();
    Serial.println(F("Access FSM - Nested Switch (return) pattern ready (ESP32)."));
