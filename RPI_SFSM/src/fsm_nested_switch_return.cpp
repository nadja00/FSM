 ...
 *
 * Access Control FSM - Nested Switch (return) Pattern - Raspberry Pi / Linux port.
 * FSM: 4 states - IDLE, CHECKING, GRANTED, DENIED
 * Events: EV_VALID, EV_INVALID, EV_TIMEOUT
 *
 * Adaptacije u odnosu na AVR/ESP32 verziju:
 *   - UART/Serial registri -> obican stdin/stdout (bez potrebe za bit-banging-om).
 *   - Timer1/ESP.getCycleCount() -> perf_event_open() hardverski cycle counter
 *     (PERF_COUNT_HW_CPU_CYCLES), citano preko ioctl()/read() iz user-space-a.
 *   - cli()/sei() nema pravog ekvivalenta u Linux user-space-u (nema direktnog
 *     gasenja HW prekida); najbolje sto mozemo je pin_to_cpu0() da smanjimo
 *     migraciju procesa izmedju jezgara.
 *   - termios "raw" mod omogucava trenutnu reakciju na taster (bez Enter-a),
 *     isto ponasanje kao Serial Monitor na mikrokontroleru.
 *
 * Komande (posle pokretanja programa, bez pritiska na Enter):
 *   '1' -> EV_VALID
 *   '0' -> EV_INVALID
 *   't' -> EV_TIMEOUT
...
int main(void) {
    ...
    printf("\nAccess FSM - Nested Switch (return) pattern ready (Raspberry Pi / Linux).\n");
