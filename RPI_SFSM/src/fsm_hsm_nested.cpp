 ...
 * Raspberry Pi / Linux port. Vidi fsm_nested_switch_return.cpp za napomene o
 * Linux adaptaciji (perf_event_open, termios raw mod, /proc/self/status).
 *
 * FSM: IDLE, CHECKING, DENIED (top-level) + GRANTED (superstate) sa decom
 * NORMAL_ACCESS i ADMIN_ACCESS.
 ...
 *   'b' -> run automatic benchmark (1000 transitions), meri INHERITED EV_TIMEOUT trosak
...
static const StateNode state_table[NUM_STATES] = {
    ...
    /* STATE_NORMAL_ACCESS */ { state_normal_access_handle, STATE_GRANTED },
    /* STATE_ADMIN_ACCESS  */ { state_admin_access_handle,  STATE_GRANTED },
};
...
int main(void) {
    ...
    printf("\nAccess FSM - Manual HSM (NESTED, Test 4b) ready (Raspberry Pi / Linux).\n");
