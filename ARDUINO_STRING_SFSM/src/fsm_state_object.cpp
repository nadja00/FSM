...
static uint8_t start_state_handle(uint8_t event) {
    if (event == M_FOUND) return M_STATE;
    return START;
}

static uint8_t m_state_handle(uint8_t event) {
    if (event == INV_CHAR) return START;
    if (event == A_FOUND) return A_STATE;
    return M_STATE;
}

static uint8_t a_state_handle(uint8_t event) {
    if (event == INV_CHAR) return START;
    if (event == T_FOUND) return T_STATE;
    return START;
}
